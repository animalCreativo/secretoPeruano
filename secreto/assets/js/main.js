jQuery(document).ready(function ($) {
    ...
    function OsMainMenu_opened() {
        $(".nav-hamburger").each(function () {
            $(this).on('click', function (event) {
                event.preventDefault();
                var $this = $(this);
                if (!($this.parents(".main-nav").hasClass('main-menu-opened'))) {
                    $this
                        .parents(".main-nav")
                        .addClass('main-menu-opened');
                }
                else {
                    $this
                        .parents(".main-nav")
                        .removeClass('main-menu-opened')
                    setTimeout(function () {
    ...
    function OsNavSearch_toggle() {
        $(".main-nav").each(function () {
            ...
            $(this).find(".search-trigger-show").on('click', function (event) {
                ...
                if (!($triggerShow.parents(".main-nav").hasClass('nav-search-opened'))) {
                    $triggerShow
                        .parents(".main-nav")
                        .addClass('nav-search-opened')
                    if ($triggerShow.parents(".main-nav").hasClass('main-menu-opened')) {
                        $triggerShow
                            .parents(".main-nav")
                            .removeClass('main-menu-opened')
                        setTimeout(function () {
                            $triggerShow.parents(".main-nav")
                                .find('.nav-main-menu.small-screen .menu-dropdown > a')
                                .removeClass('menu-trigger')
                                .siblings('ul').hide();
                        }, 400);
